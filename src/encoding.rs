...
mod test_encoder {
	...
	#[test]
	fn just_frames() {
		case(convert(b"1001001001001010"), &[0x10, 0x92, 0x4a]);
	}

	#[test]
	fn mixes() {
		case(convert(b"1100110011111111111101010101"), &[0x08, 0xcc, 0xcc, 0x08, 0x55]);
	}
...
mod test_iterable {
	...
	fn with_frames() {
		...
		assert_eq!(&[0x14, 0x55, 0x55, 0x50], &*encode(src).collect::<Vec<_>>());
